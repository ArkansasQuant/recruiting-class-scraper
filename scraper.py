...
async def click_load_more_until_complete(page, year: int) -> list:
    ...
    while click_count < max_clicks:
        # Get current player count using correct selector
        current_players = await page.locator('li.rankings-page__list-item').count()
        
        # Try to find and click "Load More" button with correct selector
        load_more_button = page.locator('a.rankings-page__showmore')
    ...
    # Extract all player profile URLs using correct selector
    print(f"\n🔗 Extracting player profile URLs...")
    player_links = await page.locator('li.rankings-page__list-item a.rankings-page__name-link').all()
    ...
    # Debug: Save screenshot if no players found
    if len(player_urls) == 0:
        print(f"  ⚠️ No players found on page for {year}.")
        print(f"     Taking screenshot to check for bot blocking...")
        screenshot_path = OUTPUT_DIR / "diagnostics" / f"blocked_debug_{year}.png"
...
async def main():
    ...
    if all_players:
        ...
        # Exit with error code so GitHub Actions marks as failed
        import sys
        sys.exit(1)
